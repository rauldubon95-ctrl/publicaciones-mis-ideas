...
export default function AdminVentasLibrosPage() {
  ...
  const reenviarEnlace = useCallback(async (id: string) => {
    ...
    try {
      const r = await fetch(`/api/admin/ventas-libros/${id}/reenviar`, {
        method: "POST",
      });
      ...
      if (r.ok) {
        setReenvios((prev) => ({
          ...prev,
          [id]: { estado: "ok", mensaje: `Enviado a ${d.email ?? "el comprador"}` },
        }));
      } else {
        setReenvios((prev) => ({
          ...prev,
          [id]: { estado: "error", mensaje: d.error ?? "No se pudo reenviar" },
        }));
      }
    } catch {
      setReenvios((prev) => ({
