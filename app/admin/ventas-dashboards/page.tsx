...
interface Venta {
  ...
  tablero: { titulo: string; slug: string };
}
...
export default function AdminVentasDashboardsPage() {
  ...
  const reenviarEnlace = useCallback(async (id: string) => {
    ...
    try {
      const r = await fetch(`/api/admin/ventas-dashboards/${id}/reenviar`, {
        method: "POST",
      });
  ...
  const cargar = useCallback(() => {
    ...
    fetch(`/api/admin/ventas-dashboards${params}`)
      .then((r) => {
        if (r.status === 401) {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
      <nav className="text-sm text-zinc-400 mb-6 flex items-center gap-1">
        ...
        <span>/</span>
        <span className="text-zinc-600">Ventas de tableros</span>
      </nav>
      ...
      <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">
            Pedidos de compra de tableros Excel vía PayPal.
          </p>
        </div>
        {datos && (
            ...
            <table className="w-full text-sm">
              <thead className="bg-zinc-50 border-b border-zinc-200">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Tablero</th>
                  ...
                  <th className="text-right px-4 py-3 text-xs font-semibold text-zinc-400 uppercase tracking-wider">Acción</th>
                </tr>
              </thead>
                    ...
                    <td className="px-4 py-3 text-right whitespace-nowrap">
                      {v.estado === "COMPLETADO" ? (
                        <div className="inline-flex flex-col items-end gap-1">
                          <button
                            onClick={() => reenviarEnlace(v.id)}
                            disabled={reenvios[v.id]?.estado === "enviando"}
                          ...
                          >
                            {reenvios[v.id]?.estado === "enviando" ? "Enviando…" : "Reenviar enlace"}
                          </button>
                          {reenvios[v.id]?.estado === "ok" && (
                            <span className="text-[11px] text-emerald-600">{reenvios[v.id]?.mensaje}</span>
                          )}
                          {reenvios[v.id]?.estado === "error" && (
                            <span className="text-[11px] text-red-600">{reenvios[v.id]?.mensaje}</span>
                          )}
                        </div>
                      ) : (
