...
export default async function ComprarDashboardExitoPage({ searchParams }: Props) {
  ...
  if (pedido_id && paypalOrderId) {
    const pedido = await prisma.pedidoDashboard.findUnique({
      where: { id: pedido_id },
      select: {
        id: true, estado: true, tokenAcceso: true, tableroId: true, emailComprador: true,
        tablero: { select: { titulo: true, slug: true } },
      },
    });
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-24 text-center">
        ...
        <p className="text-zinc-500 mb-8 text-sm leading-relaxed">
          Tu pago está siendo confirmado. Recibirás el enlace de acceso por correo en unos minutos.
        </p>
        <Link href="/dashboard" className="btn-primary">Ver más tableros</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-24 text-center">
      ...
      <p className="text-zinc-500 leading-relaxed mb-2">
        Ya tienes acceso al tablero <strong className="text-zinc-800">{titulo}</strong>.
      </p>
