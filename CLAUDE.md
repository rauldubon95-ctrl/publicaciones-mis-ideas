...
| `AI` | Workers AI binding | Chat: `@cf/meta/llama-3.1-8b-instruct-fp8-fast` (sesión 38; modelo central en `workers/sociologia/src/config.ts` → `CHAT_MODEL`). Embeddings: `@cf/baai/bge-m3` (multilingüe, 1024 dims; sesión 39, migrado desde `bge-large-en-v1.5` que era inglés y rendía mal sobre español; constante `EMBEDDING_MODEL` en `config.ts`). **Requiere re-vectorizar** desde `/admin/embed-backfill` tras el deploy. |
