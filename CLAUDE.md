...
| ~~Factorizar `MuroPago`/`MuroLibro`/`MuroRecurso`/`MuroDashboard`~~ (**CERRADO sesión 26**) | `components/MuroPagoBase.tsx` concentra la lógica/markup; los 4 quedan como envoltorios delgados (597→287 líneas). Cero cambio de comportamiento/estilos. | ~~Baja~~ |
| `wrangler` 3.x con CVEs en dev-tooling (detectado sesión 29) | `workers/sociologia`: `wrangler@3.x` arrastra `esbuild`/`undici`/`ws` vulnerables (2 altas + 3 moderadas) vía `miniflare`, su emulador local. Solo afecta `wrangler dev`/CI local — **no** al Worker desplegado en el edge de Cloudflare (ese corre en `workerd`, sin este árbol de deps). El job "Auditar dependencias (worker IA)" de `supply-chain.yml` lleva semanas en rojo por esto (no bloquea merges porque no hay branch protection). Se cierra subiendo a `wrangler@4.86+` — major aplazado, ya documentado en varias sesiones; requiere sesión dedicada con prueba de humo del Worker. | Media |
...
| ~~`config/prompts/v1.1.txt` desconectado~~ (**ELIMINADO sesión 28**) | Worker usa SYSTEM_PROMPT en `prompts.ts`, no este archivo. Archivo borrado con `git rm`. | ~~Baja~~ |
...
*Sesión 27 (cont.) — **Mejoras técnicas de indexación/SEO** [rama `claude/seo-indexacion`]. El usuario reportó ~62 URLs "sin indexar" en Search Console (mayormente "Descubierta: actualmente sin indexar" = 60). Diagnóstico: NO es bug ni penalización — sitio joven, Google descubrió las ~70 URLs del sitemap (funciona) pero indexa lento por falta de autoridad/tiempo; `robots.txt` correcto, nada bloqueado. Datos reales: 34 publicaciones + 18 recursos + 4 cómics + 3 libros + 4 categorías. Se implementaron las 3 mejoras técnicas (additive, sin tocar nada funcional): (1) **Sitemap con fechas reales** (`app/sitemap.ts`): los listados y categorías ya no mandan `lastModified: new Date()` (que decía "todo cambió hoy" y erosionaba la señal); ahora usan la fecha real del contenido más nuevo de cada sección (`maxFecha`) y, por categoría, la del artículo más reciente. (2) **JSON-LD `BreadcrumbList`** (helper `breadcrumbJsonLd` en `lib/seo.ts`) en publicaciones/libros/recursos/cómics/categorías + **`Organization`** en el layout raíz + **JSON-LD `CreativeWork` en cómics** (antes no tenían ninguno). (3) **Enlazado interno**: el Footer (en todas las páginas) ahora enlaza a las 5 secciones (Publicaciones/Libros/Recursos/Cómics/Servicios), antes solo a /privacidad y /donar. Los artículos ya tenían breadcrumb visible + enlace a categoría + relacionados. Palancas grandes quedan del lado del usuario (no código): "Solicitar indexación" en GSC + conseguir autoridad/enlaces + tiempo. Gates: `tsc`+`build`(Turbopack) limpios. Verificación: build local + logs de producción tras merge (Preview no sirve páginas con DB, §18).*
...
*Sesión 28 — **Auditoría de seguridad externa (completa): C1, H1–H4, M1–M5, B1–B5 + zombie** [todo MERGEADO A MAIN directo]. Auditoría provista por tercero (`7159b32c-security_audit_report.html`). **CRÍTICO (C1)**: token premium IA pasó de HMAC estático a token con expiración de 1h (`{hmac}.{jti}.{exp}`); backward-compat con token hex legado durante transición. **ALTOS (H1–H4)**: H1 — guard criptográfico en `proxy.ts` para `/api/admin/*` (verifica cookie admin antes de llegar al handler); H2 — eliminados todos los fallbacks `?? ADMIN_SECRET` en `lib/secrets.ts` y `proxy.ts` (solo `SESSION_SIGNING_SECRET`); H3 — Worker URL leída de env var `WORKER_URL` (no hardcodeada en repositorio público); H4 — rate-limit fail-close por IP + por email en las 4 rutas de compra (5/h IP, 3/h email). **MEDIOS (M1–M5)**: M1 — rate-limit en `/api/libros/descargar` (10/min); M2 — `checkGlobalRateLimit` Worker fail-close; M3 — validación magic bytes PDF + sanitización nombre de archivo en upload; M4 — Google Fonts eliminados del CSP (ya self-hosted); M5 — cookie de acceso a libros ahora usa `lib_SHA256(libroId)[0:8]` (opaco, no slice en claro); `nombreCookieLibro()` exportada en `lib/accesoLibro.ts`. **BAJOS (B1–B5)**: B1 — email de artículo premium corregido: "funciona indefinidamente" → "válido por 30 días" (`lib/resend.ts`); B2 — delay aleatorio 50-150ms en `/api/health` y `/api/health/deep` cuando no autorizado (elimina timing side-channel); B3 — `capturarOrdenPayPal` ya no propaga body de error de PayPal en la excepción (solo loguea status + primeros 200 chars internamente); B4 — Dependabot: grupo `seguridad` con `applies-to: security-updates` en ambos ecosistemas npm (bypassa cooldown de 7 días para alertas CVE); B5 — eliminados `python-requests`, `curl`, `wget` de `BOT_UA_PATTERNS` (herramientas legítimas de dev/CI). **Zombie**: `config/prompts/system/v1.1.txt` eliminado con `git rm` (0 referencias en el código — confirmado por grep). `tsc`+`build`(Turbopack) limpios. Commits en `main`: `308f3ce`(H1-H3) + `45937e9`(H4) + `27ce4af`(M1-M5) + commit de esta sesión (B1-B5+zombie+docs).*
...
*Sesión 29 (cont., 2026-07-11/12) — **Incidente de checkout PayPal en producción real: causado y resuelto en la misma sesión.** El usuario reportó que el checkout pedía demasiada información y empujaba a iniciar sesión en PayPal en vez de mostrar el formulario de tarjeta directo. Fix 1 (`868fddb`): `lib/paypal.ts` migra de `application_context` (deprecado) a `payment_source.paypal.experience_context` con `landing_page: "GUEST_CHECKOUT"` + `shipping_preference: "NO_SHIPPING"` (ver §15). **Este cambio rompió las 4 rutas de compra** (`/api/comprar`, `/api/libros/comprar`, `/api/recursos/comprar`, `/api/dashboard/comprar`): PayPal devuelve el link de redirección con `rel: "payer-action"` en vez de `rel: "approve"` cuando se usa `payment_source.paypal`, y el código solo buscaba `"approve"` → 502 "Error al conectar con PayPal" en producción real durante ~10 minutos (confirmado en `get_runtime_errors`: 3 ocurrencias, 16:53–17:00 UTC). Fix 2 (`a20f6b7`, mismo día): acepta ambos `rel`. **Diagnóstico hecho con evidencia, no con suposiciones**: se usó `mcp__Vercel__get_runtime_errors`/`get_runtime_logs` para leer el stack trace real y el `lastDeployment` exacto de cada error — así se distinguió el bug real (mío) del "demasiados intentos" (rate-limit H4 disparado por las propias pruebas del usuario, no relacionado) y de una tercera queja ("mi PayPal se colapsó") que, revisando los logs de las siguientes 24h, **no tenía ningún error nuevo desde el fix 2** — quedó sin reproducir con evidencia de servidor (posible confusión del usuario entre intentos, o algo del lado de la cuenta PayPal ajeno al código). **Housekeeping adicional**: se liberó manualmente (DELETE dirigido por `clave` exacta, sin tocar el mecanismo) un bloqueo de rate-limit en `RateLimitDb` que el propio usuario había activado probando repetidas veces. **Lección para sesiones futuras**: al cambiar la forma del *request* a una API externa, revisar también la forma de la *respuesta* que se consume — PayPal (y APIs similares) pueden cambiar campos de la respuesta según qué variante del request se use. Ambos fixes en `main`, verificados `READY` en Vercel antes de decirle al usuario que probara.*
*Sesión 30 (cont., 2026-09-03) — **Conversión de moneda referencial (sin comprometer seguridad)** [rama `claude/conversion-monedas`, **NO mergeada a main hasta OK del usuario + Preview verde**]. El usuario pidió mostrar precios en varias monedas para su audiencia latinoamericana + europea sin romper la elegancia editorial y sin comprometer la seguridad. Implementado un sistema **puramente visual/referencial** — el cobro real sigue siendo USD via PayPal, sin cambios de esquema, sin cambios de lógica de pago. **Monedas soportadas** (`lib/monedas.ts`): USD (default) + MXN, EUR, COP, GTQ + principales sudamericanas ARS, CLP, PEN, BRL. Cada moneda con nombre, símbolo, locale BCP-47 (Intl.NumberFormat), bandera emoji, país. **Fuente de tasas** (`lib/monedas-server.ts` → `obtenerTiposCambio`): consulta 1 vez al día `https://open.er-api.com/v6/latest/USD` (público, sin API key, sin límite razonable, cubre 150+ monedas incluyendo las latinoamericanas — frankfurter.app no cubría COP/GTQ/PEN); cacheado 24h con `unstable_cache` + tag `tipos-cambio` para invalidación futura; timeout 8s via `fetchConTimeout`; **fail-safe** con snapshot fallback hardcoded (`TASAS_FALLBACK` en lib/monedas.ts) si la API cae → el visitante NUNCA ve el sitio sin precios. **Filtramos siempre** la respuesta a los códigos conocidos (no confiamos en árbol externo). Endpoint público `/api/tipos-cambio` (`app/api/tipos-cambio/route.ts`) devuelve JSON `{base, tasas, actualizadoAt, fuente: "api"|"fallback"}` con Cache-Control CDN. **Selector de moneda** (`components/SelectorMoneda.tsx`, client): dropdown compacto en Header (bandera + código), lista con todas las monedas mostrando nombre + país + código, activo con check amber; persiste en cookie `moneda_ref` (NO-httpOnly, path=/, SameSite=Lax, Secure en HTTPS, 1 año) — el server la lee vía `next/headers`; cambio dispara `router.refresh()` para que Server Components re-rendericen precios; validación con `esMonedaValida()` en cliente y servidor para blindar contra cookies manipuladas. **Componentes de precio**: (a) `components/PrecioConConversion.tsx` (server) — usado en páginas server (libros, recursos, publicaciones); (b) `components/PrecioConConversionClient.tsx` (client) — usado dentro de client components (los 4 muros de pago). Ambos con 3 tamaños (sm/md/lg), modo `soloConversion` para evitar redundancia cuando otro elemento ya muestra el USD (usado en muros: el heading ya tiene el precio USD, solo añadimos la línea "≈ MX$85 · referencial" debajo). Muestran `US$X.XX` grande + línea pequeña gris `≈ MX$85 · referencial` (o similar); si la moneda seleccionada es USD, cero ruido visual (solo el precio USD). Formato con `Intl.NumberFormat` según locale de cada moneda: COP/CLP/ARS 0 decimales (números grandes), resto 2 decimales; fallback al `symbol + toFixed()` si el locale no está disponible en el runtime. **Integración**: Header (SelectorMoneda visible en escritorio y en menú móvil desplegable); página `/libros` (lista, sm sin nota); página `/libros/[slug]` (detalle, sm con conversión); página `/recursos` (lista); los 4 muros de pago (MuroLibro/MuroPago/MuroRecurso/MuroDashboard reciben `precioCentavos` y lo pasan a MuroPagoBase → conversión aparece bajo el heading sin duplicar el USD). Los muros de dashboards y publicaciones muestran el precio como antes en el heading; la conversión aparece bajo. **SEGURIDAD (revisión propia)**: (a) precio de cobro NUNCA viene del cliente — sigue siendo `precioCentavos` del servidor via Prisma, como siempre; (b) la moneda seleccionada por el visitante NO se envía a los endpoints `/comprar` (que siguen calculando el monto desde la DB); (c) API externa se consulta ÚNICAMENTE server-side → la IP del visitante nunca llega a open.er-api.com; (d) filtrado estricto de la respuesta contra `CODIGOS_MONEDAS` conocidos; (e) cookie sin datos personales, solo un código de moneda validado; (f) NO se añaden hosts al CSP (todo el fetch es server-side); (g) fallback hardcoded es un snapshot de tasas — NO son secretos, son datos públicos de mercado; (h) URL de la API es pública documentada, no requiere clave. **Cero cambios de esquema, cero cambios de pagos, cero cambios de auth**. Aviso al usuario final: cada precio con conversión lleva "· referencial" y el `PagoSeguroInfo` de sesión anterior sigue diciendo "El cobro se realiza en USD via PayPal". Nueva ruta `/api/tipos-cambio` pública (info pública de mercado, sin auth). Zombie: eliminado `frame-src https://${supabaseHost}` residual del CSP en la sesión anterior (verificado que ningún iframe apunta al bucket) — parte del endurecimiento continuado. Gates locales: `npx tsc --noEmit` sin errores; `npm run build` limpio (Turbopack, 54 rutas OK, TS 3.4s). **Rollback**: revertir el merge de la rama `claude/conversion-monedas` — restablece el sitio a solo USD, sin efectos colaterales. PENDIENTE: verificación en Preview de Vercel + OK del usuario para mergear.*
*Sesión 30 (cont., 2026-08-30) — **Fix visor PDF (pdfjs-dist) + PayPal transparencia + endurecimiento CSP** [directo a `main`, autorizado por el usuario tras mergear el rediseño]. El iframe con visor nativo del navegador NO cargaba en muchos entornos (móviles no lo renderizan inline; sandbox lo bloquea en algunos casos). Reemplazado por **PDF.js (pdfjs-dist v4.10.38)** que renderiza cada página en `<canvas>` como galería navegable — exactamente la UX "pasar página a página" que pidió el usuario. Nuevo `components/PdfReader.tsx`: dos modos (página única con navegación anterior/siguiente + miniaturas + zoom 50-250% + teclado; scroll continuo con IntersectionObserver de pre-render a 500px), pantalla completa via Fullscreen API, descarga preservada, manejo de errores con fallback a descarga directa, spinner de carga, canvas escalado a devicePixelRatio para nitidez retina. **Seguridad del visor**: `pdfjs-dist@^4.10.38` incluye fix del CVE-2024-4367 (XSS via FontMatrix, roto en pre-4.2); `getDocument({isEvalSupported:false})` desactiva eval; el worker corre en mismo origen (Turbopack lo empaqueta y sirve desde /_next); import dinámico → PDF.js no va al bundle inicial, solo se carga cuando el usuario abre un cómic-PDF; try/catch alrededor del worker import → si Turbopack fallara con `?url` (`@ts-expect-error` documentado), PDF.js cae a "fake worker" en main thread (más lento pero funcional). **CSP endurecido en `proxy.ts`**: añadido `worker-src 'self' blob:` (solo mismo origen — no permite workers de terceros); **eliminado** `https://${supabaseHost}` del `frame-src` que había añadido en la primera fase del rediseño (ya no lo necesita: PDFs se renderizan en canvas, no en iframe → superficie de ataque reducida). Neto: CSP más restrictivo, no menos. **Transparencia de PayPal** (`components/PagoSeguroInfo.tsx`, nuevo): card discreta con badge de candado + texto claro para tranquilizar al usuario final ("El cobro lo procesa PayPal · Nunca vemos ni almacenamos tus datos de tarjeta · Puedes pagar con tarjeta sin cuenta PayPal · Pago aparece como PayPal en tu extracto"), `<details>` expandible con más info. Integrado en **MuroPagoBase** (bajo el formulario → aplica automáticamente a los 4 muros: artículos, libros, recursos, dashboards) y en `/donar` (bajo el `FormularioDonacion`). NO expone webhook IDs, endpoints internos, secretos ni IDs — solo información que un usuario final necesita. **Revisión rápida de seguridad**: (a) `titulo` renderizado en el visor viene de la DB y React escapa por defecto (no dangerouslySetInnerHTML); (b) `pdfUrl` viene de una URL validada al guardarse (endpoint paginas verifica que empiece por el path del bucket comics); (c) detección `esPdf()` usa comparación literal + endsWith seguro (`.split("?")[0]` protege de queries manipulables); (d) no se añaden nuevas cookies, headers ni endpoints; (e) el flujo de admin upload PDF sigue con el flow presigned-URL de sesión 30 (isAdminAuthorized + whitelist MIME + sanitización de nombre). Gates: `pdfjs-dist` es paquete oficial de Mozilla ampliamente auditado; Vercel instalará en el build; sin node_modules local, se confía en el Preview del deploy tras el push a main. Rollback disponible: `backup/main-antes-rediseno-2026-08-30`.*
*Sesión 30 (2026-08-30) — **Rediseño visual completo del frontend (editorial/académico)** [rama `claude/raul-dubon-website-redesign-ndp0uv`, **NO mergeada a main hasta OK del usuario + Preview verde**]. El usuario pidió rediseñar la interfaz con estética editorial (sobria, académica, minimalista) tomando como referencia visual una captura estilo revista. Cero cambios de schema ni de lógica de backend — solo frontend + una extensión trivial de tipos permitidos en un endpoint admin. (1) **Header renovado** (`components/Header.tsx`): navegación centrada, buscador desplegable (envía a `/publicaciones?q=`), botón "Suscribirme" prominente (redondeado, negro), acento amber en el hover del logo, subrayado activo animado. Menú móvil rediseñado. (2) **Home completa** (`app/page.tsx`): hero editorial split 2-columnas con headline serif grande ("Pensar lo social para transformar la realidad."), párrafo introductorio, `SubscriptionForm` variant="hero" inline con `id="suscribirme"` (target del botón del Header), y a la derecha una ilustración SVG original inline (`components/HeroIlustracion.tsx`, sin stock images: cuaderno abierto con diagrama en árbol, libro con título grabado, planta con hojas, taza con vapor, pluma inclinada — paleta marfil/verde salvia/ámbar). Debajo: chips de categorías + grid 4-columnas de publicaciones destacadas (variante nueva del `PublicacionCard`) + tarjeta grande "Libros y recursos" (3 subtarjetas: Libros/Recursos/Cómics con iconos) + CTA de suscripción de cierre. Sidebar de Novedades preservado. (3) **Footer oscuro** (`components/Footer.tsx`): 3 columnas — identidad + iconos sociales (Twitter/LinkedIn/Email) / navegación en 2 subcolumnas / newsletter con SubscriptionForm variant="dark" (input amber-focused, botón amber-600). Barra inferior con copyright + privacidad + apoyar. (4) **`PublicacionCard` rediseñado** (`components/PublicacionCard.tsx`): 3 variantes (grid/destacado/lista), todas con **portada editorial generada por categoría** (gradiente cromático + inicial serif gigante como marca decorativa + categoría en tracking uppercase) — cero stock images, cero uploads. Tiempo de lectura estimado del resumen. API compatible con todos los call-sites existentes (Home, Publicaciones, Categorías). (5) **`SubscriptionForm` con variantes** (`components/SubscriptionForm.tsx`): default/dark/hero. Todos con honeypot preservado. Reutilizable en cualquier contexto sin duplicar código. (6) **`PdfReader` — visor PDF integrado** (`components/PdfReader.tsx`): iframe + toolbar propia (título, zoom −/+, descarga, abrir en pestaña, pantalla completa via Fullscreen API con fallback). Usa visor nativo del navegador (sin dependencias externas, sin PDF.js — más simple y confiable). Sandbox seguro. Responsive (70/80/85vh según pantalla). (7) **Cómics soporta PDF sin schema change**: `app/comics/[slug]/page.tsx` detecta si la primera "página" es un PDF (URL termina en `.pdf` O caption === "__pdf__") → renderiza `PdfReader` a ancho completo (max-w-5xl); si no, sigue con `ComicReader` tradicional. Admin (`app/admin/comics/editar/[id]/page.tsx`) permite subir PDFs con `accept="...,application/pdf"` — al detectar `file.type === "application/pdf"`, se guarda automáticamente con `caption="__pdf__"` para que el frontend lo reconozca. Endpoint `app/api/admin/comics/[id]/presigned-url/route.ts` amplía `TIPOS_PERMITIDOS` para incluir `application/pdf` (única modificación de backend en toda la sesión, additive y aislada). Grid del admin muestra placeholder PDF (icono + "Ver PDF") en lugar de intentar renderizar la URL como imagen. Grid público (`app/comics/page.tsx`) muestra badge "PDF" y placeholder editorial cuando no hay portada real. **PENDIENTE para sesión siguiente**: generación automática de portada desde página 1 del PDF (requiere `pdfjs-dist`/`pdf-lib` server-side + fields nuevos en el modelo `Comic` — mejor en sesión dedicada). Por ahora el admin puede subir el PDF y funcionar el visor; la portada es un placeholder editorial. (8) **Listados rediseñados** con eyebrow amber-700 + h1 serif tracking-tight + descripción: `/publicaciones` (con búsqueda `?q=` + filtro categoría chips activo/inactivo + limpiar filtros + grid 3-col), `/libros` (portadas 2/3 aspect ratio con sombra de estantería + fallback compuesto tipográficamente con el título del libro), `/recursos` (grid con portada abstracta de "papel apilado" + badge Premium), `/comics` (grid con detección PDF + badge), `/categorias/[slug]` (header más rico). (9) **CSP actualizado** (`proxy.ts`): añadido `https://${supabaseHost}` a `frame-src` para permitir embeber PDFs de Supabase Storage en el iframe del `PdfReader`. Cambio additive (no elimina hosts existentes). (10) **Zombie eliminado**: `components/CentroCategoriasGrid.tsx` (0 usos tras el rediseño de la Home). Gates: **no se pudo correr `tsc`/`build` localmente** — el contenedor de esta sesión no tiene `node_modules` (mismo caveat que sesión 29). Se confía en el Preview de Vercel al pushear. **NADA se mergea a `main` hasta que el usuario apruebe** (regla #3). Prompt para siguiente sesión (si el usuario decide continuar la línea): añadir generación automática de portada desde página 1 del PDF (backend Node + fields en schema Comic).*
*Sesión 29 (2026-07-11) — **Retomada tras ~1 mes sin actividad. Revisión de estado + pasada de seguridad + papeleo de dependencias.** Sin cambios de producto; todo en `main` vía PRs de Dependabot ya existentes. (1) **Diagnóstico de estado**: producción sana (Vercel: último deploy `READY`, 0 errores runtime en 7d; Supabase advisors: solo `INFO` deny-by-default, 0 `WARN`/`ERROR`; `npm audit`=0; 0 Issues abiertos). Se detectó que hubo un **fix de emergencia sin documentar** el 17-jun (commit `7608376`, ya en main): el hardening H2 de sesión 28 había quitado el fallback `ADMIN_PASSWORD → ADMIN_SECRET`, y si Vercel solo tenía la variable legacy `ADMIN_SECRET` el login de admin quedaba roto (devolvía `undefined`). Se restauró el fallback en `lib/secrets.ts` — es seguro porque el riesgo real de H2 estaba en `SESSION_SIGNING_SECRET` (token de sesión), no en la contraseña de acceso. (2) **3 PRs de Dependabot revisados y mergeados** (squash, a `main`): **#38** `actions/checkout` v6→v7 (Action, sin tocar npm); **#39** `@cloudflare/workers-types` (tipos worker, dev-only); **#40** 8 dependencias menores/parche del sitio — **Next 16.2.7→16.2.9, Tailwind+`@tailwindcss/postcss` 4.3.0→4.3.1**, `@supabase/supabase-js` 2.107→2.108.2, `recharts` 3.8.1→3.9.0, `resend` 6.12.4→6.14.0, `@tailwindcss/typography` 0.5.19→0.5.20, `@types/node` parche. Los 3 tenían Preview de Vercel `READY` y el check "Auditar dependencias (web)" en verde; se verificó antes de mergear que ninguno *introducía* problemas nuevos. (3) **Hallazgo real de la pasada de seguridad**: el job **"Auditar dependencias (worker IA)"** de `.github/workflows/supply-chain.yml` lleva **fallando desde antes de esta sesión** (confirmado en runs de `main` del 14-jun y en los 3 PRs recién mergeados) — `workers/sociologia` sigue en `wrangler@3.x` (major aplazado a propósito, ver deuda técnica), que arrastra `esbuild`/`undici`/`ws` con 2 vulnerabilidades **altas** y 3 moderadas a través de `miniflare` (el emulador local de `wrangler dev`). **Alcance real: bajo** — esas piezas viven solo en la herramienta de desarrollo/CI, no en el Worker desplegado (que corre en `workerd`, el runtime de Cloudflare, sin ese árbol de dependencias npm); no afecta a los visitantes de rauldubon.org. **Gap de proceso**: no hay branch protection que exija que el check pase, así que ha estado en rojo semanas sin que nadie lo notara — los merges de dependabot/Claude a `main` no se bloquean por esto. Queda documentado como deuda técnica (tabla §11) con la solución (`wrangler@4.86+`, sesión dedicada con prueba de humo) y NO se tocó en esta sesión (es un major, fuera del alcance de "revisar y mergear PRs de parche"). (4) **Housekeeping de CLAUDE.md**: dominio marcado como conectado a Vercel (ya lo estaba desde sesión 21, la nota decía "pendiente"); versión de Next.js/Tailwind actualizada en la tabla de estado; fila "Factorizar los 4 Muros" y regla #19 actualizadas a **CERRADO** (se había hecho en sesión 26 pero el texto seguía pidiéndolo como pendiente). Gates: no se pudo correr `tsc`/`build` en el contenedor de esta sesión (Node 22 vs `engine-strict: 20.x` del `.npmrc`, y sin red para reinstalar) — se confió en el Preview `READY` de Vercel (que sí compila con Node 20) como gate, igual que en sesiones anteriores cuando el build local no estaba disponible. PENDIENTE para el usuario: ninguna acción urgente; el único ítem nuevo es de prioridad media (wrangler) y no afecta producción.*
