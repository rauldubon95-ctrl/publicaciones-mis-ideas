...
export default function MuroDashboard({ tableroId, titulo, precioCentavos }: Props) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    setCargando(true);
    try {
      const res = await fetch("/api/dashboard/comprar", {
        method: "POST",
  ...
  return (
    <div className="mt-6 border border-amber-200 bg-gradient-to-br from-amber-50 to-white rounded-2xl p-8 shadow-sm">
      ...
      <p className="text-center text-sm text-zinc-500 mb-6 max-w-md mx-auto leading-relaxed">
        Acceso permanente al tablero{" "}
        <span className="text-zinc-700 font-medium">«{titulo}»</span>. El enlace de acceso te llega por correo
        para que puedas abrir las tablas y descargar el Excel desde cualquier dispositivo.
      </p>
