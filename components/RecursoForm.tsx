...
export default function RecursoForm({ recurso }: Props) {
  ...
  async function handleFileHtml(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file.name.toLowerCase().endsWith(".html") && !file.name.toLowerCase().endsWith(".htm")) {
      setError("Solo se permiten archivos .html"); return;
    }
    if (file.size > 5 * 1024 * 1024) { setError("El archivo no puede superar 5 MB"); return; }
    setCargandoHtml(true);
    const texto = await file.text();
    setContenido(texto);
    if (!esEdicion && !titulo) setTitulo(file.name.replace(/\.(html|htm)$/i, "").replace(/[-_]/g, " "));
      ...
      <div className="grid gap-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            ...
            <div>
              <input ref={fileRef} type="file" accept=".html,.htm" className="hidden" onChange={handleFileHtml} />
              <button type="button" disabled={cargandoHtml} onClick={() => fileRef.current?.click()}
                ...
                {cargandoHtml ? "Cargando…" : "Importar .html"}
              </button>
            </div>
          </div>
